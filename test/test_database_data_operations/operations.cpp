...
void tearDown(void) {
  DB.remove(sample_json_id,sample_json_barcode);
}
...
void test_exists_returns_false(void){
  TEST_ASSERT_FALSE(DB.exists("000000"));
}
...
void test_get_json_from_id_returns_correctly(void){
    ...
    TEST_ASSERT_EQUAL_STRING(apiJsonDoc["product_name"], databaseJsonDoc["name"]);
}
...
void test_sets_correctly(void) {
    ...
    TEST_ASSERT_EQUAL_STRING( value, databaseJsonDoc["quantity"].as<const char*>());
}
