...
const uint32_t LONG_PRESS_DURATION = 2000; // duration threshold of a long press
// Don't register button presses that have happened this many ms before we checked for input
// Tlis is so that if btn1 is pressed while we're waiting for btn2, it doesn't get registered next time we check for btn1
